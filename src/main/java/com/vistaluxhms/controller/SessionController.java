...
public class SessionController {
	...
	public ModelAndView create_create_session_detail(@ModelAttribute("SESSION_MASTER_OBJ") SessionEntity sessionEntity,
			@ModelAttribute("SESSION_OBJ") SessionDetailsEntity sessionDetailsEntity, BindingResult result,
	...
	public ModelAndView create_create_session_detail(@RequestParam("sessionId") Integer sessionId,
			@ModelAttribute("SESSION_OBJ") SessionDetailsEntityDTO sessionDetailsEntityDTO, BindingResult result,
			final RedirectAttributes redirectAttrib) {
		UserDetailsObj userObj = getLoggedInUser(); // Retrieve logged-in user details
		SessionDetailId sessionDetailId = new SessionDetailId(sessionId, sessionDetailsEntityDTO.getRoomCategoryId(),
				sessionDetailsEntityDTO.getMealPlanId());
	...
	public ModelAndView view_session_list(@ModelAttribute("SESSION_FILTER_OBJ") SessionFilterDTO sessionFilterDTO,
	...
	public ModelAndView view_add_session_form(@ModelAttribute("SESSION_MASTER_OBJ") SessionEntity sessionEntity,
	...
	public ModelAndView view_add_session_form(@RequestParam("sessionId") Integer sessionId,
			@ModelAttribute("SESSION_DETAIL_OBJ") SessionDetailsEntityDTO sessionObj, BindingResult result) {
		UserDetailsObj userObj = getLoggedInUser();
		SessionEntity sessionEntity = sessionService.findSessionById(sessionId);
		 ...
		 * SessionDetailsEntityDTO newSessionDetailsEntityDTO = new
		 * SessionDetailsEntityDTO();
		 * Optional<SessionDetailsEntity> existingSessionDetailsEntity
		 * =sessionService.findSessionDetailsEntity(sessionEntity.getSessionId(),
		 * activeRoomCategory.getRoomCategoryId(),mealPlanKey);
		 * if(existingSessionDetailsEntity.isPresent()) {
		 * System.out.println("Room Category " +
		 * activeRoomCategory.getRoomCategoryName() + "   Key: " + mealPlanKey +
		 * ", Value: " + VistaluxConstants.MEAL_PLANS_MAP.get(mealPlanKey));
		 * newSessionDetailsEntityDTO.updateVOFromEntity(existingSessionDetailsEntity.
		 * get());
		...
		for (MasterRoomDetailsEntity activeRoomCategory : ACTIVE_ROOM_LIST) {
			...
			for (Integer mealPlanKey : VistaluxConstants.MEAL_PLANS_MAP.keySet()) {
				...
				Optional<SessionDetailsEntity> existingSessionDetailsEntity = sessionService
						.findSessionDetailsEntity(sessionEntity.getSessionId(), roomCategoryId, mealPlanKey);
		...
		modelView.addObject("mealPlans", VistaluxConstants.MEAL_PLANS_MAP); // {1: "EPAI", 2: "CPAI", 3: "MAPAI", 4:
																			// "APAI"}
	...
	public ModelAndView view_session_rate_mapping_form(
		...
		List<SessionRateMappingEntity> sessionRateMappingEntities = sessionService
				.filterSessionRateMappingBySessionId(sessionRateMappingEntityDTO.getSessionId());
		List<SessionRateMappingEntityDTO> listSessionRateMappingEntitiesDTO = generateSessionRateMappingDTO(
				sessionRateMappingEntities);
	...
	private List<SessionRateMappingEntityDTO> generateSessionRateMappingDTO(
			List<SessionRateMappingEntity> sessionRateMappingEntities) {
		...
		while (filteredSessionRateIterator.hasNext()) {
			SessionRateMappingEntity sessionRateMappingEntity = (SessionRateMappingEntity) filteredSessionRateIterator
					.next();
	...
	public ModelAndView create_create_session_rate_mapping(
			@ModelAttribute("SESSION_RATE_MAP_OBJ") SessionRateMappingEntityDTO sessionRateMappingEntityDTO,
			BindingResult result, final RedirectAttributes redirectAttrib) {
		ModelAndView modelView = new ModelAndView();
		...
		if (conflictExists) {
			...
			modelView.setViewName("redirect:/manage_sessionmap?sessionRateTypeId="
					+ sessionRateMappingEntityDTO.getSessionRateTypeId() + "&View");
		}
		return modelView;
	}
	...
	public ModelAndView view_view_session_map_details(
			@ModelAttribute("SESSION_RATE_MAP_OBJ") SessionRateMappingEntityDTO sessionRateMappingEntityDTO,
			BindingResult result, final RedirectAttributes redirectAttrib) {
		SessionRateMappingEntity sesisionRateMapping = sessionService
				.findSessionRateMappingEntityById(sessionRateMappingEntityDTO.getSessionRateTypeId());
	...
	public ModelAndView view_delete_session_map_details(
			@ModelAttribute("SESSION_RATE_MAP_OBJ") SessionRateMappingEntityDTO sessionRateMappingEntityDTO,
			BindingResult result, final RedirectAttributes redirectAttrib) {
		SessionRateMappingEntity sesisionRateMapping = sessionService
				.findSessionRateMappingEntityById(sessionRateMappingEntityDTO.getSessionRateTypeId());
	...
	public ModelAndView delete_delete_session_rate_mapping(
			@ModelAttribute("SESSION_RATE_MAP_OBJ") SessionRateMappingEntityDTO sessionRateMappingEntityDTO,
			BindingResult result, final RedirectAttributes redirectAttrib) {
		ModelAndView modelView = new ModelAndView();
