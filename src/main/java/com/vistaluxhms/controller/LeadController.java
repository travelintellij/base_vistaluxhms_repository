...
public class LeadController {
    ...
    public ModelAndView view_add_lead_form(@ModelAttribute("LEAD_OBJ") LeadEntityDTO leadEntityDTO,
    ...
    public ModelAndView create_create_lead(@ModelAttribute("LEAD_OBJ") LeadEntityDTO leadRecorderObj,
        ...
        if (result.hasErrors()) {
            ...
            // salesService.findSalesPartnerById(leadRecorderObj//.getLeadSource().getSalesPartnerId());
            ...
            if (leadRecorderObj.isLeadCreationClientInformed()) {
                ...
                if (leadRecorderObj.isNotifyWhatsapp()) {
                    ...
                    // CHANGED: Capture WhatsAppResult instead of fire-and-forget.
                    // If WhatsApp fails (wrong template, missing config, API error),
                    // the error message is passed to the frontend via "WhatsAppError" flash
                    // attribute.
                    // The lead creation itself is NOT blocked — only the notification failed.
    ...
    private void notifyLeadCreationTargetAudience(LeadEntityDTO leadRecorderObj, String templateName, boolean isCreated,
        ...
        if (emailClientNotifyActive) {
            ...
            String guestDetails = "Adults:" + leadRecorderObj.getAdults() + " Children: "
                    + (leadRecorderObj.getCwb() + leadRecorderObj.getCnb() + leadRecorderObj.getCompChild());
            String emailSubject = generateSubject(leadRecorderObj, guestDetails, leadReferenceNumber, isCreated);
            String servicesList = "";
            if (leadRecorderObj.isFit())
                servicesList = servicesList + "Stay";
            if (leadRecorderObj.isGroupEvent())
                servicesList = servicesList + "| Group ";
            if (leadRecorderObj.isMarriage())
                servicesList = servicesList + " | Marriage";
            if (leadRecorderObj.isOthers())
                servicesList = servicesList + " | Others";
    ...
    private String generateSubject(LeadEntityDTO leadRecorderObj, String guestDetails, String leadReferenceNumber,
            boolean isCreated) {
        String emailSubject;
        if (isCreated) {
            emailSubject = "Query Created with Query Id: " + leadReferenceNumber + " | " + guestDetails + " | Check In "
                    + formatter.format(leadRecorderObj.getCheckInDate()) + " | Check Out "
                    + formatter.format(leadRecorderObj.getCheckOutDate());
        } else {
            emailSubject = "Query Updated with Query Id: " + leadReferenceNumber + " | " + guestDetails + " | Check In "
                    + formatter.format(leadRecorderObj.getCheckInDate()) + "- | Check Out "
                    + formatter.format(leadRecorderObj.getCheckOutDate());
        }
        return emailSubject;
     ...
     * UserDetailsObj userObj = getLoggedInUser();
     * ClientObj client =
     * clientService.find_ClientBy_Id(leadRecorderObj.getContactId());
     * Map<String, Object> smsValuesMap=new HashMap<String, Object>();
     * smsValuesMap.put("CONTACT_NAME",client.getClientName());
     * smsValuesMap.put("QueryId","Q-"+ leadRecorderObj.getLeadId()+ "-" +
     * leadRecorderObj.getLeadSourceShortName());
     ...
     * 
     * String message =
     * UdanChooUtil.notificationMessagesList().getProperty(UdanChooConstants.
     * QUERY_REGISTRATION_MSG);
     * SMS smsMessage = new SMS();
     * smsMessage.setTo(client.getMobile());
     * smsMessage.setMessage(message);
     * int returnCode
     * =reminderService.sendSms(String.valueOf(smsMessage.getTo()),smsMessage.
     * getMessage());
     * return returnCode;
     * }
     * 
    ...
    public ModelAndView view_filter_leads(@RequestParam(defaultValue = "0") String page,
        ...
        Page<LeadEntity> pageLeadsFilteredRecords = leadService.filterLeads(pageNum, pageSize, filterObj.getLeadOwner(),
                sortBy, filterObj, isAdmin);
    ...
    private List<LeadEntityDTO> generateFilteredLeadsVo(Page<LeadEntity> pagedResult) {
        ...
        while (filteredLeadsIterator.hasNext()) {
            ...
            leadsVO.setStatusName(
                    commonService.findWorkLoadStatusById(leadEntity.getLeadStatus()).getWorkloadStatusName());
    ...
    public ModelAndView form_view_editlead(@ModelAttribute("LEAD_OBJ") LeadEntityDTO leadRecorderVO,
            BindingResult result) {
        ...
        /*
         * Map<Integer, String> activeUsersMap = (Map<Integer, String>)
         * modelView.getModel().get("ACTIVE_USERS_MAP");
        ...
        leadRecorderVO
                .setClientName(clientService.findClientById(leadRecorderVO.getClient().getClientId()).getClientName());
        // TODO following db call is also done inside form_register_newlead as well.
        // this can be reduced. Think it over.
        // leadRecorderVO.setStatusName(commonService.find_DealStatusById(leadRecorderVO.getLeadStatus()).getWorkloadStatusName());
        /*
         * List<UdnDealStatusVO> lead_wl_statusList =
         * commonService.find_All_Active_Status_Workload_Obj(VistaluxConstants.
         * WORKLOAD_LEAD_STATUS);
         * Map<Integer, String> leadStatusMap = (Map<Integer, String>)
         * lead_wl_statusList.stream().collect(
         * Collectors.toMap(UdnDealStatusVO::getWorkloadStatusId,
         * UdnDealStatusVO::getWorkloadStatusName));
         * modelView.addObject("LEAD_STATUS_MAP", leadStatusMap);
         */
    ...
    public ModelAndView edit_edit_lead(@ModelAttribute("LEAD_OBJ") LeadEntityDTO leadRecorderObj, BindingResult result,
    ...
    public ModelAndView form_view_lead_followup_details(@ModelAttribute("LEAD_OBJ") LeadEntityDTO leadRecorderObj,
            @ModelAttribute("LEAD_FOLLOWUP_OBJ") Leads_Followup_VO leadFollowupVO, BindingResult result) {
        ...
        List<Leads_Followup_Entity> listFollowUpRecords = leadService
                .findLeadFollowupByLeadIdDesc(tgLeadEntity.getLeadId());
    ...
    private List<Leads_Followup_VO> generateListFollowUpVo(List<Leads_Followup_Entity> listFollowUpRecords) {
        ...
        while (filteredLeadsFollowUpIterator.hasNext()) {
            ...
            leadsFollowUpVO
                    .setUserName(userDetailsService.findUserByID(leadsFollowUpEntity.getUpdatedBy()).getUsername());
            leadsFollowUpVO.setFormattedFollowUpTime(
                    DateTimeFormatter.ofPattern("dd/MMM/yyyy HH:mm").format(leadsFollowUpEntity.getFollowuptime()));
            leadsFollowUpVO.setFormattedNextFollowUpTime(
                    DateTimeFormatter.ofPattern("dd/MMM/yyyy HH:mm").format(leadsFollowUpEntity.getNextfollowuptime()));
    ...
    public ModelAndView create_create_lead_followup(@ModelAttribute("LEAD_OBJ") LeadEntityDTO leadRecorderObj,
            @ModelAttribute("LEAD_FOLLOWUP_OBJ") Leads_Followup_VO leadFollowupVO, BindingResult result,
            final RedirectAttributes redirectAttrib) {
        UserDetailsObj userObj = getLoggedInUser();
        ...
        // commonService.findDestinationById(leadEntity.getDestination()).getCityName();
        String emailBody = "Action by:  " + userObj.getUsername() + "\n";
        emailBody = emailBody + "Action Time: "
                + DateTimeFormatter.ofPattern("dd/MMM/yyyy HH:mm").format(leadFollowupVO.getFollowuptime()) + "\n";
        emailBody = emailBody + "Action Taken: " + leadFollowupVO.getResponse() + "\n\n";
        emailBody = emailBody + "Next Action Time: "
                + DateTimeFormatter.ofPattern("dd/MMM/yyyy HH:mm").format(leadFollowupVO.getNextfollowuptime()) + "\n";
        emailBody = emailBody + "Action To Be Taken: " + leadFollowupVO.getNextactionplan() + "\n";
        Mail mail = new Mail();
        mail.setSubject("Lead Followup Update: " + " Lead ID: " + leadRecorderObj.getLeadId() + " | Client : "
                + leadEntity.getClient().getClientName() + " | CheckIn : "
                + formatter.format(leadEntity.getCheckInDate()) + " | Check Out: "
                + formatter.format(leadEntity.getCheckOutDate()));
        ...
        try {
            for (int i = 0; i < emailMessageVo.getEmailToValidatedList().size(); i++) {
                ...
                // redirectAttrib.addFlashAttribute("Error","Error: While sending email.
                // Recipients setting error.<br> Please send individually or contact
                // adminstrator.");
            }
        ...
        try {
            emailService.sendEmailMessage_Notification1_MultipleRecipients_from_loggedInUser(mail, emailBody,
                    userObj.getEmail());
