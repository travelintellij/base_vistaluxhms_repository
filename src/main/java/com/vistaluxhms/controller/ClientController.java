...
public class ClientController {
    ...
    public ModelAndView view_add_client_form(@ModelAttribute("CLIENT_OBJ") ClientEntityDTO clientEntityDto,
    ...
    public ModelAndView create_create_client(@ModelAttribute("CLIENT_OBJ") ClientEntityDTO clientEntityDto,
     ...
     * public ModelAndView view_clients_list(@ModelAttribute("CLIENT_OBJ")
     * ClientEntityDTO clientEntityDto, BindingResult result) {
     * UserDetailsObj userObj = getLoggedInUser();
     ...
     * 
     * if(clientEntityDto.getCity()!=null &&
     * String.valueOf(clientEntityDto.getCity().getDestinationId()).trim().length()!
     * =0 && clientEntityDto.getCity().getDestinationId()!=0) {
     * if
     * (!commonService.existsByDestinationIdAndCityName(clientEntityDto.getCity().
     * getDestinationId(), clientEntityDto.getCityName())) {
     * result.rejectValue("cityName", "city.error");
     * }
     * }
     * if (result.hasErrors()) {
     ...
     * return modelView;
     * }else {
     * List<ClientEntity> clientFilteredList =
     * clientService.filterClients(clientEntityDto);
     * List<ClientEntityDTO> clientDTOFilteredList =
     * generateClientObj(clientFilteredList);
     * modelView.addObject("CLIENT_FILTERED_LIST", clientDTOFilteredList);
     * Map<Long, String> mapSalesPartner =
     * salesService.getActiveSalesPartnerMap(true);
     * modelView.addObject("SALES_PARTNER_MAP", mapSalesPartner);
     * }
     * return modelView;
     * }
    ...
    public ModelAndView view_clients_list(@ModelAttribute("CLIENT_OBJ") ClientEntityDTO clientEntityDto,
            BindingResult result, @RequestParam(value = "page", defaultValue = "0") int page,
        ...
        if (clientEntityDto.getCity() != null
                && String.valueOf(clientEntityDto.getCity().getDestinationId()).trim().length() != 0
        ...
        while (itrClientEntity.hasNext()) {
            ...
            try {
                ...
                if (clientEntity.getCity() != null) {
                    clientEntityDto.setCityName(
                            commonService.findDestinationById(clientEntity.getCity().getDestinationId()).getCityName());
                }
    ...
    public ModelAndView view_edit_client_form(@ModelAttribute("CLIENT_OBJ") ClientEntityDTO clientEntityDto,
            BindingResult result) {
        UserDetailsObj userObj = getLoggedInUser();
    ...
    public ModelAndView view_client_details(@ModelAttribute("CLIENT_OBJ") ClientEntityDTO clientEntityDto,
            BindingResult result) {
        UserDetailsObj userObj = getLoggedInUser();
        ...
        clientEntityDto.setCityName(
                commonService.findDestinationById(clientEntity.getCity().getDestinationId()).getCityName());
        clientEntityDto.setSalesPartnerName(salesService
                .findSalesPartnerById(clientEntity.getSalesPartner().getSalesPartnerId()).getSalesPartnerShortName());
        return modelView;
    }

    @RequestMapping(value = "/getClientList", method = { RequestMethod.GET, RequestMethod.POST })
    public @ResponseBody List<ClientEntityDTO> getTags(@RequestParam String clientName) {
        ...
        for (ClientEntity entity : entityList) {
            if (entity.getClientName().toLowerCase().contains(clientName.toLowerCase())) {
                ...
                clientEntityDTO.setSalesPartnerName(salesService
                        .findSalesPartnerById(entity.getSalesPartner().getSalesPartnerId()).getSalesPartnerShortName());
