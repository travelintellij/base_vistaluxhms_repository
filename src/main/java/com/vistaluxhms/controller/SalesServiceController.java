...
public class SalesServiceController {
    ...
    public ModelAndView view_add_rate_type_form(@ModelAttribute("RATE_TYPE_OBJ") RateType_Obj rateTypeObj,
    ...
    public ModelAndView create_create_rate_type(@ModelAttribute("RATE_TYPE_OBJ") RateType_Obj rateTypeObj,
    ...
    public ModelAndView view_rate_type_listing(@ModelAttribute("RATE_TYPE_LIST") RateType_Obj rateTypeObj,
            BindingResult result) {
        ...
        // TODO Check if some one changes the url manually then it should lead to an
        // error page. not to a server error.
    ...
    public ModelAndView view_edit_rate_type_form(@ModelAttribute("RATE_TYPE_OBJ") RateType_Obj rateTypeObj,
            BindingResult result) {
        UserDetailsObj userObj = getLoggedInUser();
    ...
    public ModelAndView createEditSalesPartner(
            @ModelAttribute("SALES_PARTNER_OBJ") SalesPartnerEntityDto salesPartnerDto, BindingResult result,
            final RedirectAttributes redirectAttrib) {
        UserDetailsObj userObj = getLoggedInUser(); // Retrieve logged-in user details
        ModelAndView modelView = new ModelAndView();
        if (!commonService.existsByDestinationIdAndCityName(salesPartnerDto.getCityId(),
                salesPartnerDto.getCityName())) {
            try {
                City_Entity newCity = new City_Entity();
                newCity.setCityName(salesPartnerDto.getCityName());
    ...
    private ClientEntityDTO getSalesPartnerMappedClientDTO(SalesPartnerEntity salesPartnerEntity,
    ...
    public ModelAndView view_add_sales_partner_form(
            @ModelAttribute("SALES_PARTNER_OBJ") SalesPartnerEntityDto salesPartnerEntityDto, BindingResult result) {
        UserDetailsObj userObj = getLoggedInUser();
    ...
    public ModelAndView view_sales_partner_list(
            @ModelAttribute("SALES_PARTNER_OBJ") SalesPartnerEntityDto searchSalesPartnerObj, BindingResult result) {
        UserDetailsObj userObj = getLoggedInUser();
    ...
    public ModelAndView view_sales_partner_details(
            @ModelAttribute("SALES_PARTNER_OBJ") SalesPartnerEntityDto searchSalesPartnerObj, BindingResult result) {
        UserDetailsObj userObj = getLoggedInUser();
        ...
        SalesPartnerEntity salesPartnerEntity = salesService
                .findSalesPartnerById(searchSalesPartnerObj.getSalesPartnerId());
        searchSalesPartnerObj.updateSalesPartnerVoFromEntity(salesPartnerEntity);
        searchSalesPartnerObj
                .setCityName(commonService.findDestinationById(salesPartnerEntity.getCityId()).getCityName());
    ...
    public ModelAndView view_edit_sales_partner_form(
            @ModelAttribute("SALES_PARTNER_OBJ") SalesPartnerEntityDto salesPartnerEntityDto, BindingResult result) {
        UserDetailsObj userObj = getLoggedInUser();
    ...
    public ModelAndView edit_edit_sales_partner(
            @ModelAttribute("SALES_PARTNER_OBJ") SalesPartnerEntityDto salesPartnerDto, BindingResult result,
            final RedirectAttributes redirectAttrib) {
        UserDetailsObj userObj = getLoggedInUser(); // Retrieve logged-in user details
        ModelAndView modelView = new ModelAndView();
        ...
        if (salesRelatedServices.isSalesPartnerMobileExists(
            ...
            SalesPartnerEntity old = salesService.findSalesPartnerById(
        ...
        if (result.hasErrors()) {
            ...
            ClientEntity clientEntity = clientService
                    .findClientEntityForSalesPartnerId(salesPartnerEntity.getSalesPartnerId());
            if (clientEntity == null) {
                ...
                ClientEntityDTO updatedClientEntityDTO = getSalesPartnerMappedClientDTO(salesPartnerEntity,
                        clientEntityDTO);
                clientEntity = new ClientEntity(updatedClientEntityDTO);
                ClientEntity oldClient = clientService.findClientEntityForSalesPartnerId(
    ...
    public ModelAndView view_add_room_category_form(@ModelAttribute("ROOM_OBJ") MasterRoomDetailsEntity roomEntity,
            BindingResult result) {
    ...
    public ModelAndView create_create_room_category(
            @ModelAttribute("ROOM_OBJ") MasterRoomDetailsEntity roomDetailsEntity, BindingResult result,
            final RedirectAttributes redirectAttrib) {
        ModelAndView modelView = new ModelAndView("admin/rooms/Admin_Add_Room");
    ...
    public ModelAndView view_room_category_details(@ModelAttribute("ROOM_OBJ") MasterRoomDetailsEntity roomCategoryDTO,
            BindingResult result) {
        UserDetailsObj userObj = getLoggedInUser();
    ...
    public ModelAndView view_edit_room_form(@ModelAttribute("ROOM_OBJ") MasterRoomDetailsEntity roomCategoryDTO,
            BindingResult result) {
        UserDetailsObj userObj = getLoggedInUser();
    ...
    public ModelAndView edit_edit_room(@ModelAttribute("ROOM_OBJ") MasterRoomDetailsEntity roomCategoryDTO,
            BindingResult result, final RedirectAttributes redirectAttrib) {
        UserDetailsObj userObj = getLoggedInUser(); // Retrieve logged-in user details
        ModelAndView modelView = new ModelAndView();
    ...
    public ModelAndView view_rate_type_sessionwise(@ModelAttribute("RATE_TYPE_OBJ") RateType_Obj rateTypeObj,
            BindingResult result) {
    ...
    public ModelAndView view_share_season_sales_partner_form(
            @ModelAttribute("SALES_PARTNER_OBJ") SalesPartnerEntityDto salesPartnerEntityDto, BindingResult result) {
    ...
    public ModelAndView review_sales_partner_rate_share_form(
            @RequestParam(value = "rateSessionMappingIds", required = true) List<Integer> rateSessionMappingIds,
            @ModelAttribute("SALES_PARTNER_OBJ") SalesPartnerEntityDto salesPartnerEntityDto, BindingResult result,
            final RedirectAttributes redirectAttrib) {
        SalesPartnerEntity salesPartnerEntity = salesRelatedServices
                .findSalesPartnerById(salesPartnerEntityDto.getSalesPartnerId());
        salesPartnerEntityDto.updateSalesPartnerVoFromEntity(salesPartnerEntity);
        salesPartnerEntityDto.setEmail(salesPartnerEntity.getEmailId());
        ModelAndView modelAndView = new ModelAndView("admin/salespartner/reviewShareRateSessionForm"); // Change this to
                                                                                                       // your JSP page
        if (rateSessionMappingIds != null && !rateSessionMappingIds.isEmpty()) {
            ...
            for (Integer sessionRateMappingId : rateSessionMappingIds) {
                SessionRateMappingEntity sessionRateMappingEntity = sessionService
                        .findSessionRateMappingEntityById(sessionRateMappingId);
            ...
            sessionDetailsList.sort(Comparator.comparing(map -> {
                Map<Integer, Map<Integer, SessionDetailsEntityDTO>> sessionDetailsMap = (Map<Integer, Map<Integer, SessionDetailsEntityDTO>>) map
                        .get("sessionDetailsMap");
        ...
        modelAndView.addObject("mealPlans", VistaluxConstants.MEAL_PLANS_MAP); // {1: "EPAI", 2: "CPAI", 3: "MAPAI", 4:
                                                                               // "APAI"}
        return modelAndView;
    }
    ...
    private Map getSessionDetailMap(SessionRateMappingEntity sessionRateMappingEntity) {
        ...
        for (MasterRoomDetailsEntity activeRoomCategory : ACTIVE_ROOM_LIST) {
            ...
            Map<Integer, SessionDetailsEntityDTO> mealPlanMap = sessionDetailsMap.getOrDefault(roomCategoryId,
                    new LinkedHashMap<>());
            ...
            for (Integer mealPlanKey : VistaluxConstants.MEAL_PLANS_MAP.keySet()) {
                ...
                Optional<SessionDetailsEntity> existingSessionDetailsEntity = sessionService.findSessionDetailsEntity(
                        sessionRateMappingEntity.getSessionEntity().getSessionId(), roomCategoryId, mealPlanKey);
                ...
                newSessionDetailsEntityDTO
                        .setSessionStartDate(sessionRateMappingEntity.getStartDate().format(formatter));
    ...
    public ModelAndView send_send_sales_partner_rate_share(
            @RequestParam(value = "rateSessionMappingIds", required = false) List<Integer> rateSessionMappingIds,
            @ModelAttribute("SALES_PARTNER_OBJ") SalesPartnerEntityDto salesPartnerEntityDto, BindingResult result,
            final RedirectAttributes redirectAttrib) {
        ...
        // SessionRateMapHelperDTO sessionRateMapHelperDTO =
        // getSessionDetailsList(rateSessionMappingIds);
        // List<Map<String, Object>> sessionDetailsList =
        // sessionRateMapHelperDTO.getSessionDetailsList();
        ...
        for (Map<String, Object> sessionDetails : sessionDetailsList) {
            ...
            for (Object key : roomCategoryMap.keySet()) {
                ...
                Map<Integer, SessionDetailsEntityDTO> mealwiseSessionDetail = (Map<Integer, SessionDetailsEntityDTO>) sessionDetailsMap
                        .get(roomCategory.getRoomCategoryId());
    ...
    /*
     * private SessionRateMapHelperDTO getSessionDetailsList(List<Integer>
     * rateSessionMappingIds) {
     * SessionRateMapHelperDTO sessionRateMapHelperDTO = new
     * SessionRateMapHelperDTO();
     * RateCard rateCard = new RateCard();
     * List<Map<String, Object>> sessionDetailsList = new ArrayList<>();
     * if (rateSessionMappingIds != null && !rateSessionMappingIds.isEmpty()) {
     * // Process the selected session IDs
     ...
     * sessionService.findSessionRateMappingEntityById(sessionRateMappingId);
     * rateCard.setSeasonStartDate(formatter.format(sessionRateMappingEntity.
     * getStartDate()));
     * rateCard.setSeasonEndDate(formatter.format(sessionRateMappingEntity.
     * getEndDate()));
     * 
     * Map<String, Object> sessionDetail =
     * getSessionDetailMap(sessionRateMappingEntity);
     * if (sessionDetail != null) {
     * sessionDetailsList.add(sessionDetail);
     * }
     * }
     ...
     * (Map<Integer, Map<Integer, SessionDetailsEntityDTO>>)
     * map.get("sessionDetailsMap");
     * 
     * if (sessionDetailsMap != null && !sessionDetailsMap.isEmpty()) {
     * for (Map<Integer, SessionDetailsEntityDTO> mealPlanMap :
     * sessionDetailsMap.values()) {
     * for (SessionDetailsEntityDTO dto : mealPlanMap.values()) {
     * if (dto != null && dto.getSessionStartDate() != null) {
     * return LocalDate.parse(dto.getSessionStartDate(), formatter);
     * }
    ...
    @RequestMapping(value = "send_send_sales_partner_rate_share", params = "download", method = { RequestMethod.GET,
            RequestMethod.POST })
    @ResponseBody
    public void download_salesparter_rate(
            @RequestParam(value = "rateSessionMappingIds", required = false) List<Integer> rateSessionMappingIds,
            @ModelAttribute("SALES_PARTNER_OBJ") SalesPartnerEntityDto salesPartnerEntityDto,
